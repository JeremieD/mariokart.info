...
const graphicsRoot = "/resources/graphics/mkw/";
const V = {
  menu: {},
  combo: {},
  dominant: {},
...
whenDOMReady(() => {
  ...
  V.settings.gameVersionSelect = document.getElementById("settings-game-version");
  ...
  V.settings.gameVersionSelect.addEventListener("change", () => {
    changeGameVersion(V.settings.gameVersionSelect.value);
  }, { passive: true });
  V.settings.gameVersionSelect.addEventListener("open", () => {
  ...
  V.settings.localeSelect.addEventListener("open", () => {
    V.settings.statScaleSelect.close();
    V.settings.gameVersionSelect.close();
  }, { passive: true });
...
function drawPageTitle() {
  ...
  document.title = (getCustomName(combo) ?? combo.name) + " | MKW Combo Builder";
}
