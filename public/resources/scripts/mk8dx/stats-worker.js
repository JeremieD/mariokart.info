...
function listCombos(opts = {}) {
  ...
  const invMin   = opts.min?.[statIndex.inv]   ?? 0; const invMax   = opts.max?.[statIndex.inv]   ?? 20;
  ...
  for (let glider of gliderClasses) {
    ...
    if (combo.lvl[statIndex.inv]   < invMin   || combo.lvl[statIndex.inv]   > invMax)   continue;
    ...
    const diffs = [
      ...
      combo.lvl[statIndex.inv]   - refCombo.lvl[statIndex.inv]
    ];
